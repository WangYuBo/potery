// 检查是否已经存在，避免重复声明
if (typeof PoetryGenerator === 'undefined') {
    class PoetryGenerator {
        ...
        addWordToPoetry(word) {
            ...
            // 检查是否已经有这个字词（避免重复）
            const existingWords = Array.from(currentLineElement.querySelectorAll('.poetry-word'))
                .map(el => el.textContent);
            
            if (existingWords.includes(word)) {
                console.log('字词已存在，不重复添加:', word);
                this.showMessage('该字词已在当前行中', 'warning');
                return;
            }
    ...
    // 导出到全局作用域
    window.PoetryGenerator = PoetryGenerator;
}
